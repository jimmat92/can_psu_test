...
Edit `config-elmbpsu.xml` first if your port is not `can0` or your node is not
63. Since step 1 already set the bitrate, change `settings="125k"` to
...
### 3. Debug the CAN bus (only if step 4 comes back empty)
...
```bash
sudo dnf install can-utils     # once
candump -ta can0
```
...
Still empty? Retry at 250000 and 50000 in step 1 before suspecting the wiring.
...
### 5. Switch one branch on, then off
...
### 6. Read the voltage and current
